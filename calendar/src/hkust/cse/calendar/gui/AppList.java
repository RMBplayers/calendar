...
public class AppList extends JPanel implements ActionListener {
	...
	private void delete() {
		...
		if (apptTitle.TimeSpan().StartTime().getTime() < parent.controller.getSystemTime(today.getTime()).getTime()) {
				JOptionPane.showMessageDialog(this, "Cannot delete an event that already happened",
	...
	private void modify() {
		...
		// replace system time with user defined time
		GregorianCalendar today = new GregorianCalendar();
		if (apptTitle.TimeSpan().StartTime().getTime() < today.getTimeInMillis()) {
				JOptionPane.showMessageDialog(this, "Cannot modify an event that already happened",
						"Input Error", JOptionPane.ERROR_MESSAGE);
		} else {
