...
public class CalGrid extends JFrame implements ActionListener {
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		StyleConstants.setForeground(sab, Color.pink);
		StyleConstants.setFontFamily(sab, fontNames[0]);
	...
	JMenuBar createMenuBar() {
		...
		mi = new JMenuItem("Display time");
		mi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				TimeSetter testClock = new TimeSetter(controller);
			}
		});
		Appmenu.add(mi);
	...
	public void UpdateCal() {
		if (mCurrUser != null) {
			...
			this.setTitle(mCurrTitle + "(" + currentY + "-" + currentM + "-"
					+ currentD + ")");
			// get the appts of this month
			Appt[] monthAppts = GetMonthAppts();
			
			// first clear the prior location message
			for (int row = 0; row < 6; row++)
				for (int col = 0; col < 7; col++)
					CalCellRenderer.light[row][col] = false;
			
			// show appointment on calendar
			// daily weekly monthly 
			if (monthAppts != null) {
				for (int i = 0; i < monthAppts.length; i++) {
					// daily event
					if (monthAppts[i].getFrequency() == 1) {
						for (int row = 0; row < 6; row++) {
							for (int col = 0; col < 7; col++) {
								String temp = tableView.getModel().getValueAt(row, col).toString() ;
								if (!temp.equals("")) {
									int date = Integer.parseInt(temp);
									Timestamp future = new Timestamp(currentY-1900, currentM-1, date, 18, 0, 0, 0);
									if (future.after(monthAppts[i].TimeSpan().StartTime())) {		// if there is a appt at the specific date
										CalCellRenderer.light[row][col] = true;
									}
								}
							}
						}
					...
					// weekly event
					else if (monthAppts[i].getFrequency() == 2) {
						for (int row = 0; row < 6; row++) {
							for (int col = 0; col < 7; col++) {
								String temp = tableView.getModel().getValueAt(row, col).toString() ;
								if (!temp.equals("")) {
									int date = Integer.parseInt(temp);
									Timestamp future = new Timestamp(currentY-1900, currentM-1, date, 18, 0, 0, 0);
									if (future.after(monthAppts[i].TimeSpan().StartTime())
										&&	(future.getTime() - monthAppts[i].TimeSpan().StartTime().getTime())/1000/3600/24 % 7 == 0) {		// if there is a appt at the specific date
										CalCellRenderer.light[row][col] = true;
									}
								}
							}
					...
					// onetime or monthly event
						...
						for (int row = 0; row < 6; row++) {
							for (int col = 0; col < 7; col++) {
								...
								if (!temp.equals("")) {
									int date = Integer.parseInt(temp);
									if (date == monthAppts[i].TimeSpan().StartTime().getDate()) {		// if there is a appt at the specific date
										CalCellRenderer.light[row][col] = true;
								}
								}
							}
...
//	}
	...
	private void mouseResponse() {
		...
		CalGrid.this.setTitle(mCurrTitle + "(" + currentY + "-" + currentM
				+ "-" + currentD + ")");
		updateAppList();
		// this part is originally to change color when clicking, now we don't need it
		/*
